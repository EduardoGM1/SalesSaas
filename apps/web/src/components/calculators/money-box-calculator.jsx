...
export function MoneyAmount({ value, tone = "sale" }) {
...
export function ProposalMatrix({ proposals, terms, showTotalToday, t }) {
            ...
            <tr key={term.id || term.label}>
              ...
              {columns.map((p, i) => {
                ...
                if (!row || row.available === false) {
                  return (
                    <td key={`${term.label}-${i}`} className="money-box-plan-cell">
...
export const MONEY_BOX_DEFAULT_RESTRICTIONS = {
  minDownPct: "30",
  maxDownPct: "50",
  fc: "0",
  ff: "0",
...
export function applySavedRestrictions(cfg, setters) {
...
/**
 * Calculadora Money Box. Página completa usa planes de worksheetConfig (PMT);
 * embebida en Worksheet RH recibe `terms` del Catálogo de Financiamiento.
 */
export function MoneyBoxCalculator({
  embedded = false,
  worksheetConfig,
  terms: termsProp,
  savedRestrictions = null,
  onSaveRestrictions,
  termsHintKey = "moneyBox.termsFromWorksheet",
  loading = false,
  showCurrencySelector = !embedded,
}) {
  const { t } = useI18n();
  ...
  const [minDownPct, setMinDownPct] = useState(MONEY_BOX_DEFAULT_RESTRICTIONS.minDownPct);
  const [maxDownPct, setMaxDownPct] = useState(MONEY_BOX_DEFAULT_RESTRICTIONS.maxDownPct);
  const [fcInput, setFcInput] = useState(MONEY_BOX_DEFAULT_RESTRICTIONS.fc);
  const [ffInput, setFfInput] = useState(MONEY_BOX_DEFAULT_RESTRICTIONS.ff);
  const [maxSaleInput, setMaxSaleInput] = useState(MONEY_BOX_DEFAULT_RESTRICTIONS.maxSale);
  const [roundStep, setRoundStep] = useState(MONEY_BOX_DEFAULT_RESTRICTIONS.roundStep);
  ...
  useEffect(() => {
    ...
    if (!savedRestrictions) return;
    hydratedRef.current = true;
    applySavedRestrictions(savedRestrictions, {
      setMinDownPct,
      setMaxDownPct,
      setFcInput,
      setFfInput,
  ...
  async function handleSaveRestrictions() {
    ...
    try {
      ...
      if (onSaveRestrictions) {
        await onSaveRestrictions(payload);
      } else {
        await saveSettingsPatchRemote({ moneyBoxConfig: payload });
      }
      hydratedRef.current = true;
    } catch (err) {
  ...
  const summary = t("moneyBox.restrictionSummaryV2", { count: terms.length });
      ...
      >
        <div className="money-box-restrictions-layout">
          ...
          <div className="money-box-terms-block">
            ...
            <p className="card-sub money-box-terms-sub">{t(termsHintKey)}</p>
            {terms.map((term) => (
  ...
  if (embedded) {
    return <div className="money-box-embedded">{body}</div>;
  }
  return body;
}
