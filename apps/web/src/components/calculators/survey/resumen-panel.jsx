...
function line(label, value) {
  ...
  return (
    <div className="disc-summary-line" key={label}>
      ...
      <span className="disc-summary-value">{value}</span>
    </div>
  );
}

/** Resumen automático a partir de chips seleccionados y métricas de gastos (decisión de producto documentada). */
  ...
  return (
    <div className="disc-panel">
      <div className="disc-section-head">
        <div>
          <h2 className="disc-section-title">4. Resumen</h2>
      ...
      <div className="disc-summary-grid">
        <div className="disc-summary-box">
          <h3>Motivaciones</h3>
          {motivLines.length ? (
            motivLines
          ) : (
            <p className="disc-summary-empty">
              Motivación principal, no negociables, frenos, estilo de viaje y forma de decidir.
            </p>
          )}
        </div>
        <div className="disc-summary-box">
          <h3>Experiencia con timeshare</h3>
          {tsLines.length ? (
            tsLines
          ) : (
            <p className="disc-summary-empty">
        ...
        <div className="disc-summary-box">
          ...
          {patternBits.length ? (
            patternBits.map((t) => (
              <p key={t} className="disc-summary-pattern">{t}</p>
            ))
          ) : (
            <p className="disc-summary-empty">
