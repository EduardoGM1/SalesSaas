...
/** Editor de excepciones (presentación). Se usa dentro del modal. */
...
/** Sub-característica dentro del modal (acordeón interno permitido). */
function ModuleSubfeatureRow({
  ...
  return (
    <article className="admin-module-subfeature">
...
function ModuleCard({
  ...
  const [panel, setPanel] = useState(null); // 'rules' | 'children' | null
  const [pending, setPending] = useState(false);
  const [error, setError] = useState("");
  const [dirty, setDirty] = useState(false);
