...
/**
 * Comparte un expediente a todos los miembros de un chat grupal.
 * Fan-out: una fila prospect_shares por miembro (cada uno decide Guardar).
 * Una sola tarjeta access_granted en el chat grupal.
 * El Gerente ya puede leer vía RLS de grupo; igual se crea share para UX de Guardar.
 */
export async function createShareToGroup(supabase, userId, prospectId, {
  ...
  const { data: part } = await supabase
    ...
    .eq("user_id", userId)
    .maybeSingle();
  if (!part) throw new ServiceError("No perteneces a esta conversación.", 403);
  ...
  for (const sharedWithId of recipientIds) {
    const { data, error } = await supabase
      ...
      .single();
    if (error) {
      // Contacto no aceptado: omitir ese miembro, no tumbar el fan-out
      continue;
    }
    shares.push(data);
  }

  const prospectName = owned.name || owned.name1 || "Expediente";
  const label = PERM_LABEL[permission] || permission;
  try {
    const { sendGroupStructuredMessage } = await import("./group-chat-service.js");
    await sendGroupStructuredMessage(supabase, {
      conversationId,
      senderId: userId,
      body: `Se compartió el expediente «${prospectName}» con acceso de ${label} (cada miembro puede guardarlo).`,
      messageType: MESSAGE_TYPES.ACCESS_GRANTED,
      metadata: {
        prospect_id: prospectId,
        prospect_name: prospectName,
        prospect_code: owned.prospect_code || null,
        share_id: shares[0]?.id || null,
        shares: shares.map((s) => ({ share_id: s.id, shared_with_id: s.shared_with_id })),
        permission,
        owner_id: userId,
        group_share: true,
      },
