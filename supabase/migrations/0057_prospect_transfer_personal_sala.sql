-- ============================================================
-- 0057 — Transferencia definitiva Personal → Sala de Ventas
-- Mueve el MISMO expediente (mismo id) con todos sus recursos
-- hijos y deja rastro en el historial del workflow.
-- Empresa → Personal y Empresa A → Empresa B siguen prohibidos
-- (workspace_boundary_ok no cambia; este RPC es la única vía).
-- ============================================================
...
begin
  ...
  -- Mismo registro: solo cambia el workspace. Nunca se crea copia ni nuevo id.
  ...
  -- El expediente entra al flujo comercial de la sala (dispara 'workflow_iniciado').
...
revoke all on function public.transfer_prospect_to_sala(uuid, uuid, uuid) from public;
grant execute on function public.transfer_prospect_to_sala(uuid, uuid, uuid) to service_role;

comment on function public.transfer_prospect_to_sala(uuid, uuid, uuid) is
  'Transferencia definitiva Personal → Sala: mismo expediente, hijos incluidos, evento auditado.';
