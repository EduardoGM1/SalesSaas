...
export function SyncProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const doReconcile = async () => {
      ...
      try {
        await reconcile(sbc, useDbStore.getState().db, uid);
    ...
    const initForUser = async (userId: string) => {
      ...
      let cloudDb;
      try {
        cloudDb = await pullAll(sb, userId);
      } catch (err) {
